...
func (s *DockerDaemonSuite) TestDaemonRestartWithKilledRunningContainer(t *check.C) {
	...
	// kill the container
	runCmd := exec.Command(ctrBinary, "--address", "/var/run/docker/libcontainerd/containerd.sock", "containers", "kill", cid)
...
func (s *DockerDaemonSuite) TestDaemonRestartWithPausedRunningContainer(t *check.C) {
	...
	// kill the container
	runCmd := exec.Command(ctrBinary, "--address", "/var/run/docker/libcontainerd/containerd.sock", "containers", "pause", cid)
	if out, ec, err := runCommandWithOutput(runCmd); err != nil {
		t.Fatalf("Failed to run ctr, ExitCode: %d, err: '%v' output: '%s' cid: '%s'\n", ec, err, out, cid)
	}
...
func (s *DockerDaemonSuite) TestDaemonRestartWithUnpausedRunningContainer(t *check.C) {
	...
	runCmd := exec.Command(ctrBinary, "--address", "/var/run/docker/libcontainerd/containerd.sock", "containers", "resume", cid)
	if out, ec, err := runCommandWithOutput(runCmd); err != nil {
		t.Fatalf("Failed to run ctr, ExitCode: %d, err: '%v' output: '%s' cid: '%s'\n", ec, err, out, cid)
	}
