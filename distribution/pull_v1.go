...
func (p *v1Puller) Pull(ctx context.Context, ref reference.Named) error {
	...
	tr := transport.NewTransport(
		...
		registry.DockerHeaders(autoversion.DockerUserAgent(ctx), p.config.MetaHeaders)...,
	)
	client := registry.HTTPClient(tr)
	v1Endpoint, err := p.endpoint.ToV1Endpoint(autoversion.DockerUserAgent(ctx), p.config.MetaHeaders)
	if err != nil {
